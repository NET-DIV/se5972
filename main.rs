...
fn parse_entire_file_by_extension(file_path: &Path) -> Result<String, ()> {
    ...
    match extension.as_ref() {
        ...
        // specialized parser for markdown files
    ...
    'next_file: for file in dir {
        ...
        if model.requires_reindexing(&file_path, last_modified) {
            ...
            let content = match parse_entire_file_by_extension(&file_path) {
                ...
                //still add the skipped files to the model to prevent their reindexing in the future
    ...
    match subcommand.as_str() {
        "serve" => {
...

            {
                ...
                thread::spawn(move || {
                    let mut processed = 0;
                    //indexing thread intimidates the environment with a crash
